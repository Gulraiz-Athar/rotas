...
class YearlyViewController extends Controller
{
    public function index()
    {
        if (\Auth::check()) {
            ...
            $employee = Employee::where('is_delete', '0')->where('created_by', $created_by)->orwhere('id', $created_by)->get()->toArray();
            ...
            if (Auth::user()->type != 'company') {
                $company_setting_data = Employee::where('is_delete', '0')->Where('id', $created_by)->first();
                ...
                if (!empty($rotas)) {
                    ...
                    foreach ($rotas as $rota) {
                        ...
                        if (!empty($rota['role_id'])) {
                            $role_data = Role::find($rota['role_id']);
                            if($role_data){
                        ...
                        $count_role_id[date('Y-m-d', strtotime($date))][$rota['role_id']][$rota['location_id']][] = array(
                            'id'                => $rota['id'],
                            'roll_id_color'     => $color,
                            'roll_id'           => $roll_ids,
                            'role_id'           => $roll_ids,
            ...
            foreach ($count_role_id as $feed_key => $count_role_ids) {
                ...
                if (!empty($count_role_ids)) {
                                ...
                                foreach ($feed_location_data as $feed_user_id => $feed_user_data) {
                                    ...
                                    if (!empty($custome_salary_array) && !empty($feed_user_data['role_id'])) {
                                        if (
                                            ...
                                            $ppp = $custome_salary_array[$feed_user_data['role_id']]['custom_salary_by_hour'] . 'cus';
                                            if (!empty($custome_salary_array[$feed_user_data['role_id']]['custom_salary_by_shift'])) {
                                                ...
                                                $ppp = $custome_salary_array[$feed_user_data['role_id']]['custom_salary_by_hour'] . '+' . $custome_salary_array[$feed_user_data['role_id']]['custom_salary_by_shift'] . 'cus-shift';
                                            }
                                            ...
                                            if (!empty($default_salarys_array['salary']) && $default_salarys_array['salary_per'] == 'hourly') {
                                                ...
                                                $ppp = $default_salarys_array['salary'] . 'cus-def';
                                            }
                                        }
                                    } else {
                                        if (!empty($default_salarys_array)) {
                                            if (!empty($default_salarys_array['salary']) && $default_salarys_array['salary_per'] == 'hourly') {
                                                $daily_expence1 = $time_counter * $default_salarys_array['salary'];
                                                $ppp = $default_salarys_array['salary'] . 'def';
                                            }
                                        }
                                    }
                    ...
                    $feed_calender[$i] = array(
                        ...
                        'html' => '<div>' . $roll_cnt . '<div class=" ' . $text_color . ' opacity-50 mt-2" style=" font-size: 13px; "> <span title="' . __('Employees') . ' ' . $cnt_employee1 . '"></span> </div></div>',
                    );
                }
            }
            // dd($feed_calender);
            ...
            return view('dashboard.yearview', compact('feed_calender', 'roles', 'locations'));
        } else {
