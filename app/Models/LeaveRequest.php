...
class LeaveRequest extends Model
{
    ...
    public function getRequestResponse()
    {
        ...
        if ($leave_approval != 0) {
            ...
            if($leave_approval == 1) { $span = '<span class="badge badge-success bg-success  mr-2"><b> '.__("APPROVED").' </b></span>'; }
            if($leave_approval == 2) { $span = '<span class="badge badge-danger bg-danger  mr-2"><b> '.__("DENIED").' </b></span>'; }
            if($leave_approval == 3) { $span = '<span class="badge badge-info bg-info  mr-2"><b> '.__("DELETED").' </b></span>'; }
            ...
            $span = '<span class="badge badge-secondary bg-secondary mr-2"><b> '.__("REQUESTED").' </b></span>';
            $result = $span.''.__(' by ').' '.$ApprovedByName.' '.__(' on ').' '.$created_at.' ';
        }
        return $result;
    }
    ...
    public static function getLeaveHasPermission()
    {
        ...
        if(Auth::user()->acount_type == 2 && !empty(Auth::user()->manager_permission))
        {
            ...
            if(!empty($manager_permission))
            {
                ...
                $haspermission['leave_edit_permission'] = $manager_permission['manager_manage_leave_and_approve_leave_requests_for_other'];
