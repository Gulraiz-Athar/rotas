...
class Employee extends Model
{
    ...
    public static function getMangaerMangeLocationUser()
    {
        ...
        if(!empty($permission) && Auth::user()->acount_type == 2)
        {
            ...
            $profile_where = [];
            if(isset($permission['manage_loaction']) && !empty($permission['manage_loaction']))
            {
    ...
    public function getUsedHoliday($id = 0) 
    {
        ...
        if(!empty($usedleaves) && count($usedleaves) != 0)
        {
            foreach($usedleaves as $usedleave)
            {
                ...
                //dd($usedleave['duration']);
                //(($diff + 1) * 
                $leve_count += (double)$usedleave['duration'];
                //$leve_count += $diff + 1;
            }            
        }
        return $leve_count.' '.$leave_type;
    }
    ...
    {        
        ...
        if(!empty($confirmleaves) && count($confirmleaves) != 0)
        {
            foreach($confirmleaves as $confirmleave)
            {
                ...
                //($diff + 1) * 
                $leve_count += (double)$confirmleave['duration'];
                //$leve_count += $diff + 1;
            }            
        }
        ...
        if($annual_holiday == '') 
        {
            $remaing_day = '<sapn class="text-info"> ? '.$leave_type.'</span>';
        } else {
            $remaing_day = $annual_holiday - $leve_count;
            if ($remaing_day <= 0) 
            {
                $remaing_day = '<sapn class="text-danger">'.$remaing_day.' '.$leave_type.'</span>';
            } else {
                $remaing_day = '<sapn class="text-success">'.$remaing_day.' '.$leave_type.'</span>';
            }
        }
        return $remaing_day;
    }
    ...
    public function getWorkSchedule($id = 0, $week = 0, $location_id = 0)
    {
        ...
        if($id != 0)
        {
            ...
            foreach($week_date as $key => $date)
            {
                ...
                if(!empty($rotas)) 
                {
                    ...
                    foreach($rotas as $rota)
                    {
                        ...
                        if($login_employee->acount_type == 1 || $login_employee->acount_type == 2 || $class2 ==1) 
                        {
                            ...
                            if(Auth::user()->acount_type == 1 || $manager_add_edit_delete_rotas == 1)
                            {
            ...
            if(!empty($employee))
            {
                foreach($week_date as $key => $date)
                {
                    ...
                    $add = '';
                    if($login_employee->acount_type == 1 || $login_employee->acount_type == 2 || in_array($location_id,$manage_location1))
                    {
                        $manager_add_edit_delete_rotas = '';
                        if(!is_null(Auth::user()->manager_permission))
                        {
                        ...
                        if($manager_add_edit_delete_rotas != 0)
                        {
            ...
            $ci = 0;
            // print_r($id);
            $profile_data_af = Profile::select('custom_am_off')->Where('user_id',$id)->first();
            
            foreach($week_date as $key => $date)
            {                
                if(!empty(($profile_data_af['custom_am_off'])))
                {
                    $custom_am_off = json_decode($profile_data_af['custom_am_off'], true);
                    // print_r($custom_day_off);
                    if(in_array($date,$custom_am_off))
                    {
                        $tooltip = __('This is ').''.$employee['first_name'].'  '.__('Day Off');
                        $day_off_sts = ($day_off == 'hide') ? 'style="display:none;"' : '';
                        $tooltip2 = '<div class="text-center text-danger day_off_leave cus_day_off_leave" '.$day_off_sts.' data-date="'.$date.'"  title="'.$tooltip.'" data-placement="top" data-html="true" data-toggle="tooltip">'.__('AM Off').'</div>';
            ...
            $ci = 0;
            // print_r($id);
            $profile_data_pf = Profile::select('custom_pm_off')->Where('user_id',$id)->first();
            
            foreach($week_date as $key => $date)
            {                
                if(!empty(($profile_data_pf['custom_pm_off'])))
                {
                    $custom_pm_off = json_decode($profile_data_pf['custom_pm_off'], true);
                    // print_r($custom_day_off);
                    if(in_array($date,$custom_pm_off))
                    {
                        $tooltip = __('This is ').''.$employee['first_name'].'  '.__('Day Off');
                        $day_off_sts = ($day_off == 'hide') ? 'style="display:none;"' : '';
                        $tooltip2 = '<div class="text-center text-danger day_off_leave cus_day_off_leave" '.$day_off_sts.' data-date="'.$date.'"  title="'.$tooltip.'" data-placement="top" data-html="true" data-toggle="tooltip">'.__('PM Off').'</div>';
        ...
        if($emp_hide_rotas_hour == 0 || Auth::user()->type == 'company')
        {
            ...
            if(!empty($time_counter)) 
            {
                ...
                $working_hour = $hours.' '.__('Hours').' '.round($minute).' '.__('Minutes');
            }
        ...
        $shift_role = '<span title="'.$tooltip_role.'" >'.$count_shift.''._(' Shifts').'</span>';
    ...
    public function getWorkScheduleam($id = 0, $week = 0, $location_id = 0)
    {
        $week1 = $week;
        ...
        if($id != 0)
        {
            ...
            {                
                if(!empty(($profile_data_df['custom_day_off'])))
                {
                    ...
                    if(in_array($date,$custom_day_off))
                    {
                        $tooltip = __('This is ').''.$employee['first_name'].'  '.__('AM Off');
                        $day_off_sts = ($day_off == 'hide') ? 'style="display:none;"' : '';
                        $tooltip2 = '<div class="text-center text-danger day_off_leave cus_day_off_leave" '.$day_off_sts.' data-date="'.$date.'"  title="'.$tooltip.'" data-placement="top" data-html="true" data-toggle="tooltip">'.__('AM Off').'</div>';
    ...
    public function getWorkSchedulepm($id = 0, $week = 0, $location_id = 0)
    {
        $week1 = $week;
    ...
    public function getsecondWorkSchedule($id = 0, $week = 0, $location_id = 0)
    {
        $week1 = $week;
        $week = $week * 14;
        $week_date = Rotas::getWeekArray('Y-m-d',$week);
        ...
        if($id != 0)
        {
            ...
            $custom_day_off = '';
            $ci = 0;
            $profile_data_df = Profile::select('custom_am_off')->Where('user_id',$id)->first();
            foreach($week_date as $key => $date)
            {                
                if(!empty(($profile_data_df['custom_am_off'])))
                {

                    
                    $custom_day_off = json_decode($profile_data_df['custom_am_off'], true);
                    if(in_array($date,$custom_day_off))
                    {
                        $tooltip = __('This is ').''.$employee['first_name'].'  '.__('AM Off');
                        $day_off_sts = ($day_off == 'hide') ? 'style="display:none;"' : '';
            ...
            $custom_day_off = '';
            $ci = 0;
            $profile_data_df = Profile::select('custom_pm_off')->Where('user_id',$id)->first();
            foreach($week_date as $key => $date)
            {                
                if(!empty(($profile_data_df['custom_pm_off'])))
                {

                    
                    $custom_day_off = json_decode($profile_data_df['custom_pm_off'], true);
                    if(in_array($date,$custom_day_off))
                    {
                        $tooltip = __('This is ').''.$employee['first_name'].'  '.__('Day Off');
                        $day_off_sts = ($day_off == 'hide') ? 'style="display:none;"' : '';
    ...
    public function getsecondWorkScheduletwo($id = 0, $week = 0, $location_id = 0)
    {
        $week1 = $week;
        $week = $week * 14;
        $week_date = Rotas::getsecondweekArray('Y-m-d',$week);
        $tr='';
        $rotas_time = '';
        $table_date = [];
        $availabilitie_data = array('','','','','','','');
                    ...
                    foreach($rotas as $rota)
                    {
                        ...
                        if($login_employee->acount_type == 1 || $login_employee->acount_type == 2 || $class2 ==1) 
                        {
                            ...
                            if(Auth::user()->acount_type == 1 || $manager_add_edit_delete_rotas == 1)
                            {
                                ...
                                $delete ='<span>
                                        <a href="#" title="'.__('Delete').'" class="delete_rotas_action2 delete_rotas only_rotas bg-transparent p-0 action-item" data-ajax-popup="false" id="'.$rota['id'].'" action_url="'.route('rotas.destroy',$rota['id']).'" >
                                        <i class="fas fa-trash"></i>
                                    </a>
                                    <form method="POST" action="'.route('rotas.destroy',$rota['id']).'" id="delete-form-'.$rota['id'].'" class="d-none">
                                        <input name="_method" type="hidden" value="DELETE">
            ...
            if(!empty($employee))
            {
                foreach($week_date as $key => $date)
                {
                    ...
                    if($login_employee->acount_type == 1 || $login_employee->acount_type == 2 || in_array($location_id,$manage_location1))
                    {
                        ...
                        if($manager_add_edit_delete_rotas != 0)
                        {
                            $add = '<a href="#" class="action-item bg-primary rounded  add_rotas px-1" data-size="md" data-ajax-popup="true" data-title="'. __('Add New Shift').'" title="'. __('Add New Shift').'" data-url="'. route('rotas.createtwo',['id'=>$id,'date'=>$date]).'"><i class="ti ti-plus text-white bg-transparent"></i></a>';
                        }
                    }
    ...
    public function getthirdWorkSchedulethird($id = 0, $week = 0, $location_id = 0)
    {
        $week1 = $week;
        $week = $week * 21;
        $week_date = Rotas::getthirdweekArray('Y-m-d',$week);
        // print_r($week_date);
        $tr='';
        $rotas_time = '';
        $table_date = [];
    ...
    public function getthirdWorkScheduleduo($id = 0, $week = 0, $location_id = 0)
    {
        $week1 = $week;
        $week = $week * 21;
        $week_date = Rotas::getthirdweekArraytwo('Y-m-d',$week);
        // print_r($week_date);
        $tr='';
        $rotas_time = '';
        $table_date = [];
    ...
    public function getthirdWorkScheduletrio($id = 0, $week = 0, $location_id = 0)
    {
        $week1 = $week;
        $week = $week * 21;
        $week_date = Rotas::getthirdweekArraytrio('Y-m-d',$week);
        // print_r($week_date);
        $tr='';
        $rotas_time = '';
        $table_date = [];
                    ...
                    foreach($rotas as $rota)
                    {
                        ...
                        if($login_employee->acount_type == 1 || $login_employee->acount_type == 2 || $class2 ==1) 
                        {
                            ...
                            if(Auth::user()->acount_type == 1 || $manager_add_edit_delete_rotas == 1)
                            {
                                ...
                                $delete ='<span>
                                        <a href="#" title="'.__('Delete').'" class="delete_rotas_action3 delete_rotas only_rotas bg-transparent p-0 action-item" data-ajax-popup="false" id="'.$rota['id'].'" action_url="'.route('rotas.destroy',$rota['id']).'" >
                                        <i class="fas fa-trash"></i>
                                    </a>
                                    <form method="POST" action="'.route('rotas.destroy',$rota['id']).'" id="delete-form-'.$rota['id'].'" class="d-none">
                                        <input name="_method" type="hidden" value="DELETE">
    ...
    public function getfourWorkSchedulesolo($id = 0, $week = 0, $location_id = 0)
    {
        $week1 = $week;
        $week = $week * 28;
        $week_date = Rotas::getfourweekArraysolo('Y-m-d',$week);
        $tr='';
        $rotas_time = '';
        $table_date = [];
        $availabilitie_data = array('','','','','','','');
    ...
    public function getfourWorkScheduleduo($id = 0, $week = 0, $location_id = 0)
    {
        $week1 = $week;
        $week = $week * 28;
        $week_date = Rotas::getfourweekArrayduo('Y-m-d',$week);
        // print_r($week_date);
        $tr='';
        $rotas_time = '';
        $table_date = [];
    ...
    public function getfourWorkScheduletrio($id = 0, $week = 0, $location_id = 0)
    {
        $week1 = $week;
        $week = $week * 28;
        $week_date = Rotas::getfourweekArraytrio('Y-m-d',$week);
        // print_r($week_date);
        $tr='';
        $rotas_time = '';
        $table_date = [];
    ...
    public function getfourWorkSchedulequadra($id = 0, $week = 0, $location_id = 0)
    {
        $week1 = $week;
        $week = $week * 28;
        $week_date = Rotas::getfourweekArrayquadra('Y-m-d',$week);
        // print_r($week_date);
        $tr='';
        $rotas_time = '';
        $table_date = [];
                    ...
                    foreach($rotas as $rota)
                    {
                        ...
                        if($login_employee->acount_type == 1 || $login_employee->acount_type == 2 || $class2 ==1) 
                        {
                            ...
                            if(Auth::user()->acount_type == 1 || $manager_add_edit_delete_rotas == 1)
                            {
                                ...
                                $delete ='<span>
                                        <a href="#" title="'.__('Delete').'" class="delete_rotas_action4 delete_rotas only_rotas bg-transparent p-0 action-item" data-ajax-popup="false" id="'.$rota['id'].'" action_url="'.route('rotas.destroy',$rota['id']).'" >
                                        <i class="fas fa-trash"></i>
                                    </a>
                                    <form method="POST" action="'.route('rotas.destroy',$rota['id']).'" id="delete-form-'.$rota['id'].'" class="d-none">
                                        <input name="_method" type="hidden" value="DELETE">
    ...
    public static function getCompanyWeeklyUserSalary($week = 0, $create_by = '', $first_location = 0, $role_id = 0)
    {
        ...
        if($working_hour1 > 0)
        {
            ...
            $wm2 = 60 * $wm1 / 1;
            $wm2 = (!empty($m2)) ? $wm2 : 00 ;
            $working_hour1 =  $wh1.''.__('Hours').' '.(int)$wm2.__('Minutes');
        }
        ...
        $tr1 = '<tr class="text-center"> <td> <span>'.$working_hour1.'</span></td> <td class="min_width-170">'.$tr_hour[0].'</td> <td class="min_width-170">'.$tr_hour[1].'</td> <td class="min_width-170">'.$tr_hour[2].'</td> <td class="min_width-170">'.$tr_hour[3].'</td> <td class="min_width-170">'.$tr_hour[4].'</td> <td class="min_width-170">'.$tr_hour[5].'</td> <td class="min_width-170">'.$tr_hour[6].'</td> </tr>';
        $tr2 = '';
        if($emp_show_rotas_price == 1)
        {
            $tr2 = '<tr class="text-center"> <td><span>'.__('Wages').'</span> <span>'.User::priceFormat($weekly_money).'</span></td> <td class="min_width-170">'.User::priceFormat($tr_cost[0]).'</td> <td class="min_width-170">'.User::priceFormat($tr_cost[1]).'</td> <td class="min_width-170">'.User::priceFormat($tr_cost[2]).'</td> <td class="min_width-170">'.User::priceFormat($tr_cost[3]).'</td> <td class="min_width-170">'.User::priceFormat($tr_cost[4]).'</td> <td class="min_width-170">'.User::priceFormat($tr_cost[5]).'</td> <td class="min_width-170">'.User::priceFormat($tr_cost[6]).'</td> </tr>';
        }
    ...
    public static function getCompanyWeeklyUserSalaryone($week = 0, $create_by = '', $first_location = 0, $role_id = 0)
    {
        $week = $week * 14;
        $week_date = Rotas::getWeekArray('Y-m-d',$week);
    ...
    public static function getCompanyWeeklyUserSalarytwo($week = 0, $create_by = '', $first_location = 0, $role_id = 0)
    {
        $week = $week * 14;
        $week_date = Rotas::getsecondweekArray('Y-m-d',$week);
    ...
    public static function getCompanyWeeklyUserSalarythree($week = 0, $create_by = '', $first_location = 0, $role_id = 0)
    {
        $week = $week * 21;
        $week_date = Rotas::getthirdweekArray('Y-m-d',$week);
    ...
    public static function getCompanyWeeklyUserSalarythreeduo($week = 0, $create_by = '', $first_location = 0, $role_id = 0)
    {
        $week = $week * 21;
        $week_date = Rotas::getthirdweekArraytwo('Y-m-d',$week);
    ...
    public static function getCompanyWeeklyUserSalarythreetrio($week = 0, $create_by = '', $first_location = 0, $role_id = 0)
    {
        $week = $week * 21;
        $week_date = Rotas::getthirdweekArraytrio('Y-m-d',$week);
    ...
    public static function getCompanyWeeklyUserSalaryfour($week = 0, $create_by = '', $first_location = 0, $role_id = 0)
    {
        $week = $week * 28;
        $week_date = Rotas::getfourweekArraysolo('Y-m-d',$week);
    ...
    public static function getCompanyWeeklyUserSalaryfourduo($week = 0, $create_by = '', $first_location = 0, $role_id = 0)
    {
        $week = $week * 28;
        $week_date = Rotas::getfourweekArrayduo('Y-m-d',$week);
    ...
    public static function getCompanyWeeklyUserSalaryfourtrio($week = 0, $create_by = '', $first_location = 0, $role_id = 0)
    {
        $week = $week * 28;
        $week_date = Rotas::getthirdweekArraytrio('Y-m-d',$week);
    ...
    public static function getCompanyWeeklyUserSalaryfourquad($week = 0, $create_by = '', $first_location = 0, $role_id = 0)
    {
        $week = $week * 28;
        $week_date = Rotas::getfourweekArrayquadra('Y-m-d',$week);
