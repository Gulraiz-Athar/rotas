...
@section('content')
    <div class="dash-container">
        <div class="dash-content">
                    ...
                    <div class="row align-items-center">
                         <div class="col-md-6">
                            <div class="page-header-title">
                                <h4 class="m-b-10">{{ __('Contract') }} <?php //   dd($contracts);  ?> </h4>
                            </div>
                            <ul class="breadcrumb">
            ...
            <!-- DIT to hide, not useful/applicable to rota
                        ...
                        <div class="card-header card-body table-border-style">
                                ...
                                <table class="table mb-0 pc-dt-simple">
                                    <thead>
                                        <tr>
			                                ...
			                                @if(\Auth::user()->type !='employee' || \Auth::user()->acount_type == 1)
                            ...
                            <tr class="font-style">
                                ...
                                <td>{{ !empty($contract->contract_name)?$contract->contract_name:''}}</td>
                                 @if(\Auth::user()->type !='employee' || \Auth::user()->acount_type == 1)
                                    <td>{{ !empty($contract->employees)?($contract->employees->first_name .' '. $contract->employees->last_name):'' }}</td>
                                @endif
                                <td>{{ !empty($contract->types)?$contract->types->name:'' }}</td>
                                <td>{{ \Auth::user()->priceFormat($contract->value) }}</td>
                                <td>{{ ($contract->subject) }}</td>
                                <td>@if($contract->start_date != null) {{  \Auth::user()->dateFormat($contract->start_date )}} @endif</td>
                                <td>@if($contract->end_date != null) {{  \Auth::user()->dateFormat($contract->end_date )}} @endif</td>
                                ...
                                <!-- <td>{{  !empty(ucfirst($contract->status)) ? ucfirst($contract->status) : 'Close' }}</td> -->
                                    ...
                                    <td class="action text-right">
                                        ...
                                        @if(\Auth::user()->type=='company' || \Auth::user()->acount_type == 1 || \Auth::user()->type == 'employee' && $contract->edit_status == 'accept')
