                    ...
                    <div class="row " style="padding: 20px 25px;">
                        <div class="col-md-6" style="padding: 20px 25px;">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <p> <h6 class="d-inline-block">{{__('Employee Name :')}}</h6>
                                        <span>{{ $contract_id->employees->first_name .' '.  $contract_id->employees->last_name }}</span> </p>
                                     
                                    <p> <h6 class="d-inline-block">{{__('Contract Type :')}}</h6>
                                        <span>{{ !empty($contract_id->types)?$contract_id->types->name:'' }}</span> </p>
                                     
                                    <h6 class="d-inline-block">{{__('Salary :')}}</h6>
                                        <span>{{ \Auth::user()->priceFormat($contract_id->value) }}</span> </p>
                                    
                                    <h6 class="d-inline-block">{{__('Subject :')}}</h6>
                                        <span>{{ $contract_id->subject }}</span> </p>
                                        
                                    <h6 class="d-inline-block">{{__('Notes :')}}</h6>
                                        <span>{{ $contract_id->notes }}</span>
                                </div>
                                <div class="col-md-6 text-end">
                                    <p> <h6 class="d-inline-block">{{__('Start Date :')}} </h6>
                                    <span> @if($contract_id->start_date != null) 
                                        {{  \Auth::user()->dateFormat($contract_id->start_date )}}
                                        @endif
                                    </span> </p>
                                    <h6 class="d-inline-block">{{__('End Date :')}}</h6>
                                     <span>
                                        @if($contract_id->end_date != null) 
                                        {{  \Auth::user()->dateFormat($contract_id->end_date )}}
                                        @endif
                                    </span> 
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-6">
                                ...
                                <p> <h6 class="d-inline-block">{{__('Employee Signature  :')}} </h6>
