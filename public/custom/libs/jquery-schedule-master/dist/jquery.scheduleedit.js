...
;(function ($, window, document, undefined) {
  ...
  $.extend(Plugin.prototype, {
    ...
    create: function () {
      ...
      for (var j = 0; j < 24; j++) {
        $('<div class="jqs-grid-line"><div class="jqs-grid-hour" style="top:'+(-17+j)+'px">' + this.formatHour(j) + '</div><div class="jqs-grid-hour" style="top:'+(j+2)+'.7px">' + this.formatHour(j,'30') + '</div></div>').
          appendTo($('.jqs-grid', this.element));
      }
    ...
    openOptions: function (event, period) {
      ...
      var position = Math.round(period.position().top / this.periodPosition);
      var height = Math.round(period.height() / this.periodPosition);
      var time = '<div class="jqs-options-time">' + this.periodInitPopup(position, position + height) + '</div>';
      ...
      if (this.settings.periodColors && this.settings.periodColors.length > 0) {
        ...
        colorInput = '<div class="jqs-options-color-container d-flex flex-column">';
      ...
      // button
      var remove = '<div class="jqs-options-remove w-100 mt-2 font-weight-normal text-center py-1 rounded" style="background-color: rgba(200, 0, 0, 0.5);border-color: #f00;color: #000;"> ' + this.settings.periodRemoveButton + '</div>';
    ...
    periodInitPopup: function (start, end) {
      return '<div class="row"><div class="col text-center p-0">Start Time: </div><div class="col text-center p-0">'+this.periodFormat(start) + '</div></div><div class="row"><div class="col text-center p-0">End Time: </div><div class="col text-center p-0">' + this.periodFormat(end)+'</div></div>';
    },
     ...
     * @returns {string}
     */
    formatHour: function (hour,min='00') {
      if (this.settings.hour === 12) {
        switch (hour) {
          case 0:
          case 24:
