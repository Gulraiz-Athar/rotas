...
$(document).ready(function () {
    ...
    $(document).on('click', '.weak_go', function () {
        ...
        if (weak_left) {
            ...
            // next week starting date with format change
            next_week_startdate1 = next_week_startdate.format('D MMM YYYY');
            next_week_startdate2 = next_week_startdate.format('YYYY/MM/D');

            // alert(next_week_startdate1);
            // alert(next_week_startdate2);
...
$(document).on('change','.leave_duration', function () {
        var due_date_val = $('.leave_date_due').val();
        var start_date_val = $('.leave_date_start').val();
        // console.log(due_date_val,start_date_val);
        if (start_date_val == undefined || start_date_val == '') {
            start_date_val = due_date_val;
            $('.leave_date_start').attr('value',due_date_val);
        }
        var start_date = $('.leave_date_start').val();
        var end_date = $('.leave_date_due').val();
        var start = moment(start_date, "YYYY-MM-DD");
        var end = moment(end_date, "YYYY-MM-DD");
        var total_days = moment.duration(end.diff(start)).asDays() + 1;
        var checked_duration = $('input[name="duration"]:checked').val();
        // console.log(checked_duration); 
        $('.leave_days').val(total_days*(checked_duration));
        // $('.leave_days').attr('value',total_days*(checked_duration));
        $('.leave_days').attr('data-value',total_days,total_days*(checked_duration));
        // $('.leave_days').attr('value',total_days*($('input[name="duration"]:checked').val()==1?1:0.5));
        // $('.leave_days').attr('data-value',total_days,total_days*($('input[name="duration"]:checked').val()==1?1:0.5));
        var record_hours = getallday(start_date,end_date,total_days)
        $('.total_date_hour').html(record_hours+'<span class="clearfix"></span>');
    });